...
async def ideti_start(update, context):
...
async def gavo_foto(update, context):
...
async def gavo_pavadinima(update, context):
...
async def gavo_kategorija(update, context):
    q=update.callback_query; await q.answer(); context.user_data['kategorija']=q.data.replace("kat_","")
    await q.message.reply_text("💰 4/7 - Kaina"); return KAINA

async def gavo_kaina(update, context):
    context.user_data['kaina']=update.message.text; await update.message.reply_text("✍️ 5/7 - Aprašymas"); return APRASYMAS

async def gavo_aprasyma(update, context):
    context.user_data['aprasymas']=update.message.text; await update.message.reply_text("📍 6/7 - Vieta"); return VIETA

async def gavo_vieta(update, context):
    context.user_data['vieta']=update.message.text; await update.message.reply_text("📞 7/7 - Kontaktas"); return KONTAKTAS

async def gavo_kontakta(update, context):
    context.user_data['kontaktas']=update.message.text; d=context.user_data; laikini[update.effective_user.id]=d.copy()
    txt=f"✅ Peržiūra:\n\n📦 {d['pavadinimas']}\n🏷️ {d['kategorija']}\n💰 {d['kaina']}\n📍 {d['vieta']}\n📞 {d['kontaktas']}\n\n📝 {d['aprasymas']}\n"
    kb=[[InlineKeyboardButton("✅ TAIP, skelbti!",callback_data="patvirtinti")]]
    if d.get('foto'): await update.message.reply_photo(photo=d['foto'],caption=txt,reply_markup=InlineKeyboardMarkup(kb))
    else: await update.message.reply_text(txt,reply_markup=InlineKeyboardMarkup(kb))
    return PATVIRTINIMAS

async def patvirtinti(update, context):
    q=update.callback_query; await q.answer(); data=laikini.get(q.from_user.id) or context.user_data
    txt=f"💛 **{data['pavadinimas']}**\n\n🏷️ {data['kategorija']}\n💰 {data['kaina']}\n📍 {data['vieta']}\n📞 {data['kontaktas']}\n\n📝 {data['aprasymas']}\n\n#vilnius #auksesturgus"
    try:
        if data.get('foto'): await context.bot.send_photo(chat_id=CHANNEL_ID,photo=data['foto'],caption=txt)
        else: await context.bot.send_message(chat_id=CHANNEL_ID,text=txt)
        await q.message.reply_text(f"🚀 Paskelbta į {CHANNEL_ID}!")
    except Exception as e: await q.message.reply_text(f"❌ {e}")
    laikini.pop(q.from_user.id,None); context.user_data.clear(); return ConversationHandler.END
